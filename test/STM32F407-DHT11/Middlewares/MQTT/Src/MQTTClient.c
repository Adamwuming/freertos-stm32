...
int cycle(Client* c, Timer* timer)
{
    ...
    //if (packet_type != 65534)	printf("Cycle: Packet %d\n", packet_type);
    ...
    if (keepalive(c) < 0) return -1;
    if (packet_type < 0) return packet_type;
exit:
    if (rc == OKDONE)
        rc = packet_type;
    return rc;
...
int MQTTConnect(Client* c, MQTTPacket_connectData* options)
{
    ...
    //printf("PreSerConnect\n");
    if ((len = MQTTSerialize_connect(c->buf, c->buf_size, options)) <= 0)
        goto exit;
	//printf("PreSendPacket\n");
    if ((rc = sendPacket(c, len, &connect_timer)) != OKDONE)  // send the connect packet
        goto exit; // there was a problem
...
int MQTTSubscribe(Client* c, const char* topicFilter, enum QoS qos, messageHandler messageHandler)
{
    ...
    if (len <= 0)
        goto exit;
    if ((rc = sendPacket(c, len, &timer)) != OKDONE) // send the subscribe packet
        goto exit;             // there was a problem
...
int MQTTPublish(Client* c, const char* topicName, MQTTMessage* message)
{
    ...
    InitTimer(&timer, 13);
    countdown_ms(&timer, (message->qos == QOS0)?c->command_timeout_ms:c->command_timeout_ms*2);
