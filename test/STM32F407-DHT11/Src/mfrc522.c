...
static uint8_t MFRC_SendBuffer[2];
static uint8_t MFRC_ReadBuffer[2];
...
{
  HAL_SPI_TransmitReceive(&hspi3, MFRC_SendBuffer, MFRC_ReadBuffer,2,0xff);
}
...
void vRFIDPollTask(void *arg)
{
	...
	for(;;)
  {
		LED_Toggle(3);		
		osDelay(5000);
    PcdRequest(0x52, (uint8_t*)&cur_type);
    PcdAnticoll((uint8_t*)&cur_id);
...
unsigned char ReadRawRC(unsigned char Address)
{
     Address = ((Address<<1)&0x7E)|0x80;
		 RFID_CS_LOW;
	   MFRC_SendBuffer[0]=Address;
	   MFRC_SendBuffer[1]=MFRC522_DUMMY;
	   SPI_SendByte();
     RFID_CS_HIGH;
/*
     MF522_SCK = 0;
